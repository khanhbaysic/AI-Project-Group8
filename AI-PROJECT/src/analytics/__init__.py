...
to auto-generate next to the session ``*_details.csv``:
...
def _get_output_dir(detail_csv, output_dir=None):
    """Return the analytics output directory, creating it if needed."""
    out_dir = Path(output_dir) if output_dir is not None else Path(detail_csv).parent
    out_dir.mkdir(parents=True, exist_ok=True)
    return out_dir


def run_post_analytics(detail_csv, labels_csv=None, output_dir=None):
    """Generate heatmap and (optionally) confusion matrix from a details CSV.

    Outputs are written to *output_dir* when provided, otherwise next to the
    ``*_details.csv`` file. This keeps each video/session report isolated and
    prevents later runs from overwriting earlier analytics.
    ...
    output_dir : str or Path or None
        Destination folder for session_report.html, class_attention_heatmap.png,
        evaluation_report.txt, and confusion_matrix.png.
    """
    detail_csv = Path(detail_csv)
    analytics_dir = _get_output_dir(detail_csv, output_dir)

    # ---- 1. Session report (heatmap) ---- always runs
    try:
        from src.analytics.session_report import generate as _gen_report
        _gen_report(detail_csv, analytics_dir)
    except Exception as exc:
        print(f"[WARN] Session report generation failed: {exc}")

    # ---- 2. Confusion matrix ---- only if ground-truth labels exist
    _try_confusion_matrix(detail_csv, analytics_dir, labels_csv)
