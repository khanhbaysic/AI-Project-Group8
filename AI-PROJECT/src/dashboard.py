...
# Spoofing banner colors (vivid red)
_SPOOFING_BANNER = ("SPOOFING DETECTED", (0, 0, 200), (255, 255, 255))

class Dashboard:
    _SHOW_FOR = 5.0    # seconds an alert stays visible from first trigger
    _RESET_AFTER = 3.0 # seconds alert must be absent before it can show again

    def __init__(self, history_size=120):
        self.score_history = deque(maxlen=history_size)
        self._alert_shown_at: dict[str, float] = {}  # alert -> when it first appeared this cycle
        self._alert_last_seen: dict[str, float] = {} # alert -> last time it was generated

    def draw(self, frame, record, alerts):
        import time
        now = time.time()

        # Track when each alert was last generated by the system.
        for alert in alerts:
            already_shown = self._alert_shown_at.get(alert)
            if already_shown is None:
                # Brand new trigger — start the display clock.
                self._alert_shown_at[alert] = now
                self._alert_last_seen[alert] = now
            elif now - already_shown <= self._SHOW_FOR:
                # Still inside the visible window — keep last_seen fresh.
                self._alert_last_seen[alert] = now
            # else: display window expired — stop updating last_seen so it can age out.

        # Once last_seen is old enough, fully reset so the next trigger shows again.
        for alert in list(self._alert_shown_at):
            if now - self._alert_last_seen.get(alert, 0) >= self._RESET_AFTER:
                del self._alert_shown_at[alert]
                self._alert_last_seen.pop(alert, None)

        # Visible = tracked AND still inside the display window.
        visible_alerts = [
            a for a, t in self._alert_shown_at.items()
            if now - t <= self._SHOW_FOR
        ]
        h, w = frame.shape[:2]
        panel_w = 340
        overlay = frame.copy()
        ...
        for idx, state_name in enumerate(ALL_STATES):
            ...
            cv2.putText(frame, LABEL_EN.get(state_name, state_name), (x + 18, y0 + 11),
        ...
        if visible_alerts:
            ...
            y += 25
            for alert in list(visible_alerts)[-5:]:
                # Dim alerts that are no longer actively firing
                is_active = alert in alerts
                color = (0, 100, 255) if is_active else (100, 100, 180)
        ...
        # --- spoofing detected banner ---
        if record.get("liveness_status") == "SPOOFING":
            self._draw_custom_banner(frame, w, h, *_SPOOFING_BANNER)
    ...
    def _draw_alert_banner(frame, w, h, state):
        """Draw a large translucent alert banner for a behavioural state."""
        label, bg_color, text_color = _STATE_ALERTS[state]
        Dashboard._draw_custom_banner(frame, w, h, label, bg_color, text_color)

    @staticmethod
    def _draw_custom_banner(frame, w, h, label, bg_color, text_color):
        """Draw a large translucent alert banner with the given label and colors."""
